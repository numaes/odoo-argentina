...
class res_partner(models.Model):
    ...
    responsability_id = fields.Many2one(
        'afip.responsability', 
        'Responsability',
        #default = lambda self: self._context.get('responsability_id', '3')
        )
    document_type_id = fields.Many2one(
        'afip.document_type',
        'Document type',
        #default = lambda self: self._cr.get('afip.document_type.code', 'RUT')
        )
    document_number = fields.Char(
        'Document number', size=64,)
    iibb = fields.Char(
        'Gross Income', size=64,)
    start_date = fields.Date('Start-up Date')

    @api.onchange('document_number', 'document_type_id')
    def onchange_document(self):
        print 'Entra al onchange del Documento'
